...
class _SeeGhatsState extends State<SeeGhats> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: Text("Ghats near  "+s),),
       body:(lat != null && placedata.places2.length != 0) ? 
       GridView.builder(
        padding: const EdgeInsets.all(10.0),
        itemCount: placedata.places2.length,
        itemBuilder: (ctx,i) => Ghatitem(placedata.places2[i].name,placedata.places2[i].imagelink,placedata.places2[i].rating.toString(),placedata.places2[i].id),
